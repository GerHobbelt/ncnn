...
int Waifu2x::process(const ncnn::Mat& inimage, ncnn::Mat& outimage, const ScaleParam sparam, const int InOutType)
{
	//==============
	...
	int yoffcache = 0;
	int scale = nets[sparam.model].scale;
	int noise = nets[sparam.model].noise;
	int prepadding = nets[sparam.model].prepadding;
	...
	int post_dstchansize = outimage.cstep*outimage.elemsize;

	if (outimage.w != inimage.w*scale)
	{
		doPostScale = true;
    ...
    for (int yi = 0; yi < ytiles; yi++)
    {
        ...
        if (opt.use_fp16_storage && opt.use_int8_storage)
        {
			...
			{
				out_gpu.create(w * scale, (out_tile_y1 - out_tile_y0) * scale, (size_t)channels*2, 2, blob_vkallocator);
				//out_gpu.elemsize /= 2;
			}
			else
			{
		...
		if (doPostScale)
		{
			float dsthi = out_gpu.h*sparam.DstSize+0.5f;
			ncnn::ParamDict pd;
        ...
        // download
        {
            ...
            if (opt.use_fp16_storage && opt.use_int8_storage)	
            {
				...
				{
					out = ncnn::Mat(out_gpu.w, out_gpu.h, (unsigned short*)outimage.data + yi * scale * TILE_SIZE_Y * w * scale * channels, (size_t)channels*2,2);
				}
				else
				{
			...
			else if (i16out)
			{
				...
				unsigned short* outimgdata = (unsigned short*)(outimage.data)+(yi * scale * TILE_SIZE_Y * w * scale*channels);
				unsigned int* flou = (unsigned int*)out.data;
				...
				for (int jjh = 0; jjh < cpsiz; jjh++)
				{
					for (int iiu = 0; iiu < channels; iiu++)
					{
						outimgdata[jjh*channels + iiu] = bswap16((unsigned short)(flou[cpsiz*iiu+jjh]));
					}
				}
