...
namespace KT
{
	...
	public class MG_vtx		//s4
	{
		...
		{
			UnityEngine.Debug.Log("Bone not mod3: vtxN=" + ord + " RenN=" + Renders[0].ord + ", Flag= " + Renders[0].Info.flag.buint.ToString("X") + " at " + idxx);
		}
...
public unsafe struct bswap
{
	...
	[FieldOffset(0)] public int bint;
	
	[NonSerialized]
	[FieldOffset(0)] public fixed byte
		Text[4];
	
	[NonSerialized]
	[FieldOffset(0)] public uint buint;
	
	[NonSerialized]
	[FieldOffset(0)] public float bfloat;
	...
	*/
	...
	public void _ROL(int UnusedBits, int ro)
	{
		...
		buint = ((buint << ro) | (buint >> (-UnusedBits - ro))) >> UnusedBits;
	...
	public void _ROLfast3_1()
	{
		...
		buint = (buint & 0x7FFFFFFC | (buint >> -3)) >> 2;
	...
	public unsafe bswap(Stream reader, bool rewind)
	{
		byte[] buf = new byte[4];
		reader.Read(buf, 0, 4);
...
[StructLayout(LayoutKind.Explicit, Size = 8), Serializable]
public unsafe struct bswap8
{

	[FieldOffset(0)] public ulong bulong;
	
	[NonSerialized]
	[FieldOffset(0)] public long blong;
	
	[NonSerialized]
	[FieldOffset(0)] public double bdouble;
	
	[NonSerialized]
	[FieldOffset(0)] public uint buint0;
	
	[NonSerialized]
	[FieldOffset(0)] public bswap bsw0;
	...
	[NonSerialized]
	[FieldOffset(0)] public byte a0;
	
	[NonSerialized]
	[FieldOffset(1)] public byte a1;
	
	[NonSerialized]
	[FieldOffset(2)] public byte a2;
	
	[NonSerialized]
	[FieldOffset(3)] public byte a3;
	
	
	[NonSerialized]
	[FieldOffset(4)] public uint buint1;
	
	[NonSerialized]
	[FieldOffset(4)] public bswap bsw1;
	
	[NonSerialized]
	[FieldOffset(4)] public byte a4;
	
	[NonSerialized]
	[FieldOffset(5)] public byte a5;
	
	[NonSerialized]
	[FieldOffset(6)] public byte a6;
	
	[NonSerialized]
	[FieldOffset(7)] public byte a7;
	...
	public unsafe string _8char(uint i)
	{
		bulong = i;
		fixed(byte* hyy = Text) {
			return Encoding.ASCII.GetString(hyy, 8).Replace("\0", string.Empty);
		}
	}
	
	public unsafe string _8char()
	{
	...
	public unsafe bswap8(Stream reader, bool rewind)
	{
		byte[] buf = new byte[8];
		reader.Read(buf, 0, 8);
		fixed(byte* srcbb = &buf[0]) {
...
public class ReaderLE
{
	...
	public unsafe byte* pBas;
		
	public ReaderLE(byte[] src)
	{
		Bas = src;
	}
		
	public unsafe void Reload(byte[] nyusrc)
	{
		Bas = nyusrc;
		pBas = null;
		seek = 0;
	...
	public virtual short ri2c()
	{
		var v = Bas.rbs(seek).s0S;
		seek += 2;
		return v;
	}
		
	public virtual ushort ru2c()
	{
		var v = Bas.rbs(seek).s0;
		seek += 2;
		return v;
	}
	
	public virtual ulong ru8c()
	{
		var v = Bas.rbs8(seek).bulong;
		seek += 8;
		return v;
	}
	
	public virtual long ri8c()
	{
		var v = Bas.rbs8(seek).blong;
		seek += 8;
		return v;
	}
	...
	public unsafe virtual int ri4cp()
	{
		var v = *(int*)&pBas[seek];
		seek += 4;
	...
	public unsafe virtual uint ru4cp()
	{
		var v = *(uint*)&pBas[seek];
		seek += 4;
			
		return v;
	}
		
	public unsafe virtual short ri2cp()
	{
		var v = *(short*)&pBas[seek];
		seek += 2;
		return v;
	}
		
	public unsafe virtual ushort ru2cp()
	{
		var v = *(ushort*)&pBas[seek];
		seek += 2;
		return v;
	}
		
	public byte[] rArrayc(int n)
	{
		byte[] bb = new byte[n];
		Array.Copy(Bas, seek, bb, 0, n);
		seek += n;
		return bb;
	}
		
	public unsafe byte[] rArraycp(int n)
	{
		byte[] bb = new byte[n];
		for (int i = 0; i < n; i++) {
			bb[i] = pBas[seek + i];
	...
	public unsafe byte* CurPtr()
	{
		return pBas + seek;
	}
	
	public unsafe string rStrp(int n)
	{
		for (int i = 0; i < 256; i++) {
			if (pBas[seek + n + i] == 0) {
			
				return Encoding.UTF8.GetString(pBas + seek + n, i);
	...
	public string rStr(int n)
	{
		for (int i = 0; i < 256; i++) {
			if (Bas[seek + n + i] == 0) {
			
				return Encoding.UTF8.GetString(Bas, seek + n, i);
	...
	public unsafe string rStrp()
	{
		for (int i = 0; i < 256; i++) {
			if (pBas[seek + i] == 0) {
			
				
				var v = Encoding.UTF8.GetString(pBas + seek, i);
				seek+=(i+1);
	...
	public string rStr()
	{
		for (int i = 0; i < 256; i++) {
			if (Bas[seek + i] == 0) {
				...
				var v = Encoding.UTF8.GetString(Bas, seek, i);
				seek+=(i+1);
				return v;
	...
	public unsafe T[] ReadNArraycp<T>(int hdsz) where T : unmanaged
	{
		uint len;
		T[] retarr;
		if (hdsz > 0) {
	...
	public unsafe T ReadTcpp<T>() where T : unmanaged
	{
	...
	public unsafe byte* ReadTcp(int siz)
	{
	...
	public unsafe T ReadTcc<T>() where T : unmanaged
	{
		fixed(byte* srcbb = &Bas[seek]) {
			T v = *(T*)srcbb;
			seek += sizeof(T);
			return v;
		}
	...
	public unsafe byte* ReadTc(int siz)
	{
		fixed(byte* srcbb = &Bas[seek]) {
	...
	public unsafe T[] ReadNArrayc<T>(int hdsz) where T : unmanaged
	{
		uint len;
		T[] retarr;
		
		fixed(byte* srcbb = &Bas[0]) {
			if (hdsz > 0) {
				len = *(uint*)(srcbb + seek);
				if (hdsz == 2)
					len &= 0xFFFF;
...
public class ReaderBE : ReaderLE
{
	//bswap bswapy;
		
	public ReaderBE(byte[] src)
		: base(src)
	{
		isBE = true;
	}
		
	public override int ri4c()
	{
		var v = Bas.rbs(seek);
		v._1();
		seek += 4;
	...
	public unsafe override int ri4cp()
	{
		var v = *(bswap*)&pBas[seek];
		v._1();
		seek += 4;
	...
	public unsafe override  uint ru4cp()
	{
		var v = *(bswap*)&pBas[seek];
		v._1();
		seek += 4;
	...
	public unsafe override short ri2cp()
	{
		var v = *(bswap*)&pBas[seek];
	...
	public unsafe override  ushort ru2cp()
	{
		var v = *(bswap*)&pBas[seek];
		seek += 2;
		return v._2u();
	...
	public override ulong ru8c()
	{
		bswap8 v = Bas.rbs8(seek);
		
		seek += 8;
		return v._1u();
	}
	
	public override long ri8c()
	{
		var v = Bas.rbs8(seek);
		seek += 8;
		return v._1i();
...
public static class Extensionnyaa
{
	[MethodImpl((MethodImplOptions)0x200)]
	public static int mod4less(int inin)
	{
		return (inin^(inin<<1))&3;
		/*
		lea     eax, [rcx+rcx]
		xor     eax, ecx
		and     eax, 3
	...
	public static byte[] ReadBytes(this Stream reader, int bufsize, int bufidx, int count)
	{
		
		
		byte[] buf = new byte[bufsize];
		reader.Read(buf, bufidx, count);
		return buf;
	}
	
	public static int strlen(this byte[] b)
	{
		int bl = b.Length;
		for (int i = 0; i < bl; i++) {
			if (b[i] == 0)
	...
	public static int strlen(this byte[] b, int lim)
	{
	...
	public static int strlen(this byte[] b, int start, int lim)
	{
		if (lim < 0)
			lim = b.Length - start;
	...
	public static int strlen(this byte[] b, out string str)
	{
		...
		for (int i = 0; i < bl; i++) {
			if (b[i] == 0) {
				...
				str = Encoding.UTF8.GetString(b, 0, i);
				return i++;
			}
	...
	public static int strlen(this byte[] b, int lim, out string str)
	{
	...
	public static int strlen(this byte[] b, int start, int lim, out string str)
	{
		if (lim < 0)
			lim = b.Length - start;
		...
		for (int i = 0; i < lim; i++) {
			if (b[start + i] == 0) {
				str = Encoding.UTF8.GetString(b, start, i);
				return i++;
			}
		}
	...
	public static unsafe bswap8 rbs8(this byte[] src, int offset)
	{
		fixed(byte* srcbb = &src[offset]) {
	...
	public static unsafe long ri8(this byte[] src, int offset)
	{
		fixed(byte* srcbb = &src[offset]) {
			return *(long*)srcbb;
		}
	...
	public static unsafe ulong ru8(this byte[] src, int offset)
	{
		fixed(byte* srcbb = &src[offset]) {
			return *(ulong*)srcbb;
		}
	...
	/*	
	public static bswap ReadBswap(this BinaryReader br, int ops)
	{
		bswap BEnum = new bswap();
		BEnum.buint = br.ReadUInt32();
		if (ops == 0)
			BEnum._1();
...
//==================
[Serializable]
public class teztklaz
{
	public BoneWeight[] b1;
	public Varray[][] b2;
}
